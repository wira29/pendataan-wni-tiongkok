...
@section('content')
                    ...
                    <tbody>
                        @foreach ($transaksis as $index => $transaksi)
                            <tr>
                                ...
                                <td>
                                    @if ($transaksi->persetujuan2 == 'diproses')
                                    <button onclick="setujuiTransaksi({{ $transaksi->id }})" class="btn btn-primary">Setujui</button>
                                    @else
                                        <!-- Display button if "persetujuan2" is empty -->
                                        <span class="badge bg-success">{{ $transaksi->persetujuan2 }}</span>                                       
                                    @endif
                                </td>
                                <td>
    ...
    function setujuiTransaksi(transaksiId) {
    // Menampilkan konfirmasi
    var confirmation = confirm('Apakah Anda yakin ingin menyetujui transaksi?');
    ...
    if (confirmation) {
        ...
        $.ajax({
            type: 'POST',
            url: 'setujui-transaksi/' + transaksiId,
            data: {},
            success: function (response) {
                console.log(response); // Tampilkan respons di konsol
                if (response.success) {
                    console.log('Transaksi berhasil disetujui!');
                    
                    // Update the DataTable cell with the new status
                    const newStatus = response.status;
                    table.cell($(`#status_${transaksiId}`)).data(`<span class="badge bg-${newStatus === 'setuju' ? 'success' : 'warning'}">${newStatus}</span>`).draw();
                    ...
                    console.error('Gagal menyetujui transaksi.');
                }
            },
